...
quadrotor_common::ControlCommand PositionController::run(
  ...
  // std::cout << config.use_rate_mode << std::endl;
  ...
  command.angular_accelerations = Eigen::Vector3d::Zero();

  command.collective_thrust = command.collective_thrust * 1.0;
  // std::cout << "collective_thrust: " << command.collective_thrust << std::endl;
  
  // ROS_DEBUG("collective_thrust %f", command.collective_thrust);
