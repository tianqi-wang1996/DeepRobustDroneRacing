...

{
  ...
  autopilot_hardstop_pub_ = 
      nh_.advertise<std_msgs::Empty>("/crashed", 1);
...
void AutoPilot<Tcontroller, Tparams>::referenceStateCallback(
  ...
  if (autopilot_state_ != States::REFERENCE_CONTROL) {
    if ((reference_state_.position -
      ...
      if (ref_warning > 5) {
        autopilot_hardstop_pub_.publish(std_msgs::Empty());
        ref_warning = 0;
      }
...
void AutoPilot<Tcontroller, Tparams>::forceHoverCallback(
    const std_msgs::Empty::ConstPtr& msg) {
  state_predictor_.initialization_called_ = false;
  if (destructor_invoked_) {
    return;
  }
...
AutoPilot<Tcontroller, Tparams>::followReference(
  ...
  if ((ros::Time::now() - time_last_reference_state_input_received_) >
    ...
    // std::cout << "\n in follow: " << time_last_reference_state_input_received_ <<"\n";
    // std::cout << "\n in follow: " << ros::Time::now() - time_last_reference_state_input_received_ <<"\n";
    // std::cout << "timeout: " << reference_state_input_timeout_ << std::endl;
    setAutoPilotState(States::HOVER);
  }
