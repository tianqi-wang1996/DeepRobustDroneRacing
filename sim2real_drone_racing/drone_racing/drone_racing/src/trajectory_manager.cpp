...
bool TrajectoryManager::computeTrajectory(DroneRacing* reactive_nav,
                                          ...
                                          Eigen::Vector3d network_selection, const bool is_mb) {
  ...
  planning_length_rescaled = std::min(planning_length_max_,
                                      std::max(planning_length_min_, planning_length_ * network_selection.z()));
  ...
  bool success = trajectory_implementation_->computeTrajectory(start_state,
                                                               ...
                                                               is_mb);
  if (!success) {
    ROS_ERROR("[%s] Failed to compute local trajectory.", ros::this_node::getName().c_str());
  }
  return success;
}
...
void TrajectoryManager::visualizeTrajectory(DroneRacing* reactive_nav) {
  ...
  if (!use_mb_) {
    reactive_nav->visualizer_->visualizeTrajectory(trajectory, false, Visualizer::Color::kGreen, 0);
    return;
  }
  else{
    reactive_nav->visualizer_->visualizeTrajectory(trajectory, false, Visualizer::Color::kYellow, 0);
  }
