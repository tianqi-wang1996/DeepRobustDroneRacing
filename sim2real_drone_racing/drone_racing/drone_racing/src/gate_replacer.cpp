...
std::vector<double> GateReplacer::getGateOrientations() {

  return gates_orientations_temp_;
}
...
void GateReplacer::replaceGates() {
  ...
  for (auto &i : model_states_msg_.name) {
    if (i.find("gate") != std::string::npos) {
      ...
      // final version
      // model_state.pose.position.x = std::max(-1.1, only_gates_[goal_idx].x() + 2 * amplitude * (((double) random() / (RAND_MAX)) - 0.5));
      // model_state.pose.position.y = only_gates_[goal_idx].y() + 2 * amplitude * (((double) random() / (RAND_MAX)) - 0.5);
      model_state.pose.position.x = only_gates_[goal_idx].x();
      model_state.pose.position.y = only_gates_[goal_idx].y();
      model_state.pose.position.z = std::max(0.0, only_gates_[goal_idx].z() + 2 * amplitude * (((double) random() / (RAND_MAX)) - 0.5));
...
void GateReplacer::makeGatesMove() {
  ...
  for (auto &i : model_states_msg_.name) {
    if (i.find("gate") != std::string::npos) {
      ...
      model_state.pose.position.z = std::max(0.0, gates_positions_temp_[goal_idx].z());
      model_state.pose.orientation.w = std::cos(gates_orientations_temp_[goal_idx] / 2.0);
      model_state.pose.orientation.x = 0.0;
      model_state.pose.orientation.y = 0.0;
      model_state.pose.orientation.z = std::sin(gates_orientations_temp_[goal_idx] / 2.0);
...
void GateReplacer::loadGoalPositions() {
  ...
  only_gates_.clear();
  gates_positions_temp_.clear();
  random_phases_.clear();
  gates_orientations_temp_.clear();
  ...
  if (reverseTrack == true) {
    std::cout<<"gate reverse !!!!!!!!!!!!!!\n";
    XmlRpc::XmlRpcValue raw_positions_list_temp = raw_positions_list;
    XmlRpc::XmlRpcValue raw_orientations_list_temp = raw_orientations_list;

    for (int i = 0; i < raw_positions_list.size(); i++) {
    raw_positions_list[i] = raw_positions_list_temp[raw_positions_list.size()-i-1];
    raw_orientations_list[i] = raw_orientations_list_temp[raw_positions_list.size()-i-1];
    }
    std::cout<<"hhhhhhhhhh " << raw_positions_list[0] << std::endl;
  }
