...
def run_testing():
    ...
    gate_dae= os.path.join(ddr_dir, 'drone_racing/drone_racing/resources/race_track/real_world/gate/meshes/gate.dae')
    light_changer=os.path.join(ddr_dir, 'drone_racing/drone_racing/resources/race_track/real_world/gate/meshes/set_gate_properties.py')
    ...
    for i, bkg_img_fname in enumerate(all_images):
        ...
        os.system("python {} -xml_file {} -emission {} -ambient {}".format(
                light_changer,
                gate_dae,
                0.1*1, # Gates have little emission, 0.3
                0.7)) # 0.5

        print((bkg_img_fname, texture_goal_fname))
        print((all_gates_materials[-1], bkg_goal_fname))
        print((all_floor_materials[-1], texture_floor_fname))
