...
class DirectoryIterator(Iterator):
    def __init__(self, directory,
        ...
        if len(self.ground_truth) > 0:
            ...
            self.drone_states = np.array(self.drone_states, dtype = np.float32)
            self.ground_truth_gate = np.array(self.ground_truth_gate, dtype = np.float32)
            self.with_gt = True
        else:
            self.with_gt = False
    ...
    def _decode_experiment_dir(self, dir_subpath):
        ...
        drone_states_filename = os.path.join(dir_subpath, "drone_states.txt")
        gate_position_filename = os.path.join(dir_subpath, "gate_positions.txt")
        with_gt = False
        ...
        if os.path.isfile(labels_filename):
            ...
            max_vel = np.loadtxt(labels_filename, usecols=(3), delimiter=';')

            # state dimension
            drone_states = np.loadtxt(drone_states_filename, delimiter=';')
            # drone_states = np.loadtxt(drone_states_filename, usecols=tuple(range(9)+range(15,24)), delimiter=';')
            # # only linear velocity
            # drone_states = np.loadtxt(drone_states_filename, usecols=tuple(range(24,33)), delimiter=';')
            # only current linear velocity
            # drone_states = np.loadtxt(drone_states_filename, usecols=tuple(range(30,33)), delimiter=';')
            # only current normalized linear velocity
            # drone_states = np.loadtxt(drone_states_filename, usecols=tuple(range(30,33)), delimiter=';')/max_vel[:,None]            


            gate_position = np.loadtxt(gate_position_filename, delimiter=';')
            ...
            for frame_number, fname in enumerate(sorted_files):
                ...
                if is_valid:
                    ...
                    if with_gt:
                        ...
                        self.drone_states.append(drone_states[frame_number])
                        self.ground_truth_gate.append(gate_position[frame_number])
