...
def resnet8(img_input, drone_state_input, output_dim, scope='Prediction', reuse=False, f=0.25):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Output channel
        gate_relative_info = Dense(2+1+3)(x)

        x = concatenate([gate_relative_info,drone_state_input])
        x = Dense(int(128*f))(x)
        x = Activation('relu')(x)
        x = Dense(int(256*f))(x)
        x = Activation('relu')(x)

        final_output = Dense(output_dim)(x)



    return gate_relative_info, final_output
